...
from engines.engine_for_extract import train_one_epoch, validation_one_epoch, final_test, merge, extract_features
...
def get_args():
    ...
    parser.add_argument('--extract_features', action='store_true',
                        help='Only extract features from the last layer instead of fine-tuning')
    parser.add_argument('--feature_split', default='train', choices=['train', 'val', 'test', 'all'],
                        help='Dataset split to extract features from')
    parser.add_argument('--feature_save_dir', default='', type=str,
                        help='Directory to save extracted features')
    parser.add_argument('--feature_dtype', default='float32', choices=['float16', 'float32', 'bfloat16'],
                        help='Precision for saved features')
...
def main(args, ds_init):
    ...
    if args.feature_split in ('train', 'all'):
        dataset_train, args.nb_classes = build_dataset(is_train=True, test_mode=False, args=args)
    else:
        dataset_train = None

    if args.disable_eval_during_finetuning or args.feature_split not in ('val', 'all'):
        dataset_val = None
    else:
    ...
    #     dataset_train, num_replicas=num_tasks, rank=global_rank, shuffle=not args.extract_features
    # )
    ...
    #     drop_last=not args.extract_features,
    ...
    if args.extract_features:
        if args.enable_deepspeed:
            raise ValueError("Feature extraction is not supported when DeepSpeed is enabled.")
        if args.distributed:
            model = torch.nn.parallel.DistributedDataParallel(
                model, device_ids=[args.gpu], find_unused_parameters=False
            )
        feature_save_dir = args.feature_save_dir or (args.output_dir if args.output_dir else './output_features')
        os.makedirs(feature_save_dir, exist_ok=True)
        split_map = []
        if args.feature_split in ('train', 'all'):
            split_map.append(('train', data_loader_train))
        if args.feature_split in ('val', 'all') and data_loader_val is not None:
            split_map.append(('val', data_loader_val))
        if args.feature_split in ('test', 'all') and data_loader_test is not None:
            split_map.append(('test', data_loader_test))
        for split_name, loader in split_map:
            extract_features(
                loader, model, device, feature_save_dir, split_name, global_rank,
                bf16=args.bf16, dtype=args.feature_dtype
            )
